...
def test_comments():
    ...
    print("\n3. Getting existing comments...")
    try:
        response = requests.get(f"{BASE_URL}/api/events/{event_id}/comments")
        print(f"   Status: {response.status_code}")
        if response.status_code == 200:
    ...
    print("\n4. Adding a comment...")
    comment_data = {
        "comment": f"This is a test comment from automated test at {time.strftime('%Y-%m-%d %H:%M:%S')}"
    }
    
    try:
        headers = {"Authorization": f"Bearer {student_token}", "Content-Type": "application/json"}
        response = requests.post(f"{BASE_URL}/api/events/{event_id}/comments", json=comment_data, headers=headers)
        print(f"   Status: {response.status_code}")
        if response.status_code == 201:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if len(new_comments) > len(existing_comments):
                ...
                latest_comment = new_comments[0]  # Comments are ordered by created_at DESC
